...
- [ ] **Logo / brand mark** — needed to make the favicon and OG images (`src/app/og/[collection]/[...slug]/route.tsx`) something other than text-only. No longer blocked (see below) — just not built yet.
- [ ] Deprioritized: force-directed glossary concept-map graph (no crawlable text, no SEO payoff — revisit after the above)

> **Decided:** the site is renamed from "Marvin" to **OpenDecode** (domain: `opendecode.dev`), for domain availability — `.com`/`.dev`/`.ai`/`.org` were all unavailable for "Marvin," a common enough word to be squatted, while `opendecode.dev`/`.ai`/`.org` are all open. In-repo rename done (`appName` in `src/lib/shared.ts`, homepage hero badge, the FAQ "why the name" entry, `CLAUDE.md`/`AGENTS.md`). The People collection still profiles Marvin Minsky like any other founder of the field — only the site's own brand name changed, the person and the history didn't. **Still outside repo scope:** actually registering `opendecode.dev` (and optionally `.ai`/`.org` as redirects), pointing `NEXT_PUBLIC_SITE_URL`/Vercel's production domain at it, and renaming the GitHub repo (`creotip/marvin` → `creotip/opendecode` or similar) — `src/lib/shared.ts`'s `gitConfig.repo` still says `'marvin'` on purpose until that repo rename actually happens.
