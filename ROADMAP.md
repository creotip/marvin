...
- [x] **Newbie-readability audit** — surveyed all 22 lessons for jargon used with no plain-language anchor nearby (either genuinely unexplained, or explained once early in a long lesson then reused bare far later). Fixed 7 real gaps: "tensor" and "hyperparameter" (never defined anywhere in the course), "inductive bias" (introduced cold as a bare table header), "gradient checkpointing" and "mode collapse" (stale callbacks to a definition 150+ lines earlier), "reference-based overlap metrics" (existed only as an unexplained diagram label — added a paragraph on BLEU/ROUGE), and "instruction hierarchy" (used in a table before its one explanation later on the page).
